...
function addTab(save, inputvalue) {
  ...
  newdiv.addEventListener("pointerover", (event) => {
    makeThatSomething(event.target, "hovered", "tab");
  });
  ...
  newdiv.addEventListener("click", (event) => {
    makeThatSomething(event.target, "selected", "tab");
    sort(value);
  });
  newButton.classList.add("removeTabButton");
  newButton.addEventListener("click", () => {
    removeTab(newdiv);
    removeOneItemFromStorage("tabList", value);
  });
...
function addTodoCard(save = false, object) {
  ...
  removePriorityButtonElement.addEventListener("click", () => {
    removeTab(newContentItem);
    removeOneItemFromStorage("todoList", object.title, true);
  });
